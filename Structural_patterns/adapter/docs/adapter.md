...
![适配器设计模式](images/adapter/adapter-zh.png)
...
![适配器解决方案](images/adapter/solution-zh.png)

让我们回到股票市场程序。 为了解决数据格式不兼容的问题， 你可以为分析函数库中的每个类创建将 XML 转换为 JSON 格式的适配器， 然后让客户端仅通过这些适配器来与函数库进行交流。 当某个适配器被调用时， 它会将传入的 XML 数据转换为 JSON 结构， 并将其传递给被封装分析对象的相应方法。

##  真实世界类比

![适配器模式的示例](images/adapter/adapter-comic-1-zh.png)
...
![适配器设计模式的结构（对象适配器）](images/adapter/structure-object-adapter.png)
...
![适配器设计模式（类适配器）](images/adapter/structure-class-adapter.png)
...
![适配器模式结构的示例](images/adapter/example.png)
